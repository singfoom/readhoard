defmodule ReadhoardWeb.UserAuthTest do
  use ReadhoardWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Readhoard.Accounts
  alias Readhoard.Accounts.Scope
  alias ReadhoardWeb.UserAuth

  import Readhoard.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ReadhoardWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ReadhoardWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ReadhoardWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
